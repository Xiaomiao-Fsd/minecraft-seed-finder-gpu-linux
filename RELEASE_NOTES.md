...
- 新增女巫小屋后处理脚本：读取筛选后的 seed CSV，CUDA 扫描半径内或整世界女巫小屋，按 26.2 biome noise depth 近似 Y 找每个 seed 的最低小屋并排序输出；支持 `--whole-world` 按 Minecraft 世界边界完整搜索，支持 `--negative-y-only` 只寻找负 Y 小屋，或 `--max-y N` 限制最高 Y。
